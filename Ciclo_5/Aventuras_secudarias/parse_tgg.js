...
/**
 * parse_tgg.js
 * Parses The_Greater_Good.docx (tgg_doc.xml) → tgg_data.js
 * Run: node "Ciclo_5/Aventuras_secudarias/parse_tgg.js"
 *
 * Structure:
...
const XML_PATH  = path.join(__dirname, 'tgg_doc.xml');
const OUT_PATH  = path.join(__dirname, 'tgg_data.js');
const IMGS_DIR  = 'Aventuras_secudarias/tgg_img';
...
try {
  ...
  const docx = path.join(__dirname, "The_Greater_Good.docx");
...
function linkifyRefs(html) {
  return html.replace(/(?<![&\w="])(\d{4,5})(?!\d)(?![^<]*>)/g, (_, num) =>
    `<a class="ref-link" data-doc="tgg" data-sec="${num}">${num}</a>`
  );
}
...
const output = [
  ...
  `// Auto-generated by parse_tgg.js on ${new Date().toISOString()}`,
  `// Sections: ${sectionCount}`,
  ``,
  `var TGG_DATA = ${JSON.stringify(DATA, null, 2)};`,
  ``,
].join('\n');
...
// Spot-checks
console.log('\n=== Spot-checks ===');
['α','0001','0010','0040','0057'].forEach(k => {
  const p = sections[k];
