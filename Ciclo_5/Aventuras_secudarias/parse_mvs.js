...
/**
 * parse_mvs.js
 * Parses Medias_Vidas_Son_Verdad.docx (mvs_doc.xml) → mvs_data.js
 * Run: node "Ciclo_5/Aventuras_secudarias/parse_mvs.js"
 */

const fs   = require('fs');
const path = require('path');

const XML_PATH  = path.join(__dirname, 'mvs_doc.xml');
const OUT_PATH  = path.join(__dirname, 'mvs_data.js');
const IMGS_DIR  = 'Aventuras_secudarias/mvs_img';
...
try {
  ...
  const relsXml = execSync(`unzip -p "${path.join(__dirname, 'Medias_Vidas_Son_Verdad.docx')}" word/_rels/document.xml.rels`).toString();
...
function linkifyRefs(html) {
  return html.replace(/(?<![&\w="])(\d{4,5})(?!\d)(?![^<]*>)/g, (_, num) =>
    `<a class="ref-link" data-doc="mvs" data-sec="${num}">${num}</a>`
  );
}
...
// ─── Parse ────────────────────────────────────────────────────────────────────
console.log('\n=== Parsing Medias Vidas Son Verdad ===');
...
const output = [
  ...
  `// Auto-generated by parse_mvs.js on ${new Date().toISOString()}`,
  `// Sections: ${sectionCount}`,
  ``,
  `var MVS_DATA = ${JSON.stringify(DATA, null, 2)};`,
  ``,
].join('\n');
