...
/**
 * parse_maqd.js
 * Parses Muerte_Antes_Que_Desonhonrra.docx (maqd_doc.xml) → maqd_data.js
 * Run: node "Ciclo_5/Aventuras_secudarias/parse_maqd.js"
 */

const fs   = require('fs');
const path = require('path');

const XML_PATH  = path.join(__dirname, 'maqd_doc.xml');
const OUT_PATH  = path.join(__dirname, 'maqd_data.js');
const IMGS_DIR  = 'Aventuras_secudarias/maqd_img';
...
try {
  ...
  const relsXml = execSync(`unzip -p "${path.join(__dirname, 'Muerte_Antes_Que_Desonhonrra.docx')}" word/_rels/document.xml.rels`).toString();
...
function extractRuns(para) {
  ...
  while ((m = runRE.exec(cleaned)) !== null) {
    ...
    // Skip pure page-number runs (br + 1-3 digits)
...
function linkifyRefs(html) {
  // Linkify 4-5 digit section references
  return html.replace(/(?<![&\w="])(\d{4,5})(?!\d)(?![^<]*>)/g, (_, num) =>
    `<a class="ref-link" data-doc="maqd" data-sec="${num}">${num}</a>`
  );
}
...
// ─── Section detection ────────────────────────────────────────────────────────
// Sections: 0001–0076, plus α (alpha) and Ω (omega) intro/ending
function detectSectionStart(para) {
  if (isHorizontalRule(para)) return null;
  const plain = paragraphPlainText(para);
  if (!plain) return null;

  // Greek special sections: standalone α or Ω
  if (/^[αΩ]$/.test(plain)) return { type:'standalone', num: plain };

  // Greek inline: "α text…" or "Ω text…"
  const greekM = /^([αΩ])\s+([\s\S]+)$/.exec(plain);
  if (greekM) {
    const runs = extractRuns(para);
    return { type:'inline', num: greekM[1], restRuns: stripNumPrefix(runs, greekM[1]) };
  }
...
function stripNumPrefix(runs, num) {
  // Escape special regex chars in num (e.g. Greek letters are fine, but just in case)
...
// ─── Parse ────────────────────────────────────────────────────────────────────
console.log('\n=== Parsing Muerte Antes Que Deshonrra ===');
...
// Collect pre-section intro content to attach to 'alpha' section
// Everything before the first numbered section AND before α is discarded (title page)
// The α section captures the intro narrative
...
for (let i = 0; i < paras.length; i++) {
  ...
  if (current === null) continue; // skip title-page content before first section

  const h = paraToHtml(para);
  if (h) sections[current].push(h);
}

// Sort: α first, then numeric order, then Ω last
order.sort((a, b) => {
  if (a === 'α') return -1; if (b === 'α') return 1;
  if (a === 'Ω') return  1; if (b === 'Ω') return -1;
...
const output = [
  ...
  `// Auto-generated by parse_maqd.js on ${new Date().toISOString()}`,
  `// Sections: ${sectionCount}`,
  ``,
  `var MAQD_DATA = ${JSON.stringify(DATA, null, 2)};`,
  ``,
].join('\n');
