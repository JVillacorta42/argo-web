...
/**
 * parse_lpd.js
 * Parses La_Piel_Es_Debil_.docx (lpd_doc.xml) → lpd_data.js
 * Run: node "Ciclo_5/Aventuras_secudarias/parse_lpd.js"
 */

const fs   = require('fs');
const path = require('path');

const XML_PATH  = path.join(__dirname, 'lpd_doc.xml');
const OUT_PATH  = path.join(__dirname, 'lpd_data.js');
const IMGS_DIR  = 'Aventuras_secudarias/lpd_img';
...
try {
  ...
  const relsXml = execSync(`unzip -p "${path.join(__dirname, 'La_Piel_Es_Debil_.docx')}" word/_rels/document.xml.rels`).toString();
...
function linkifyRefs(html) {
  return html.replace(/(?<![&\w="])(\d{4,5})(?!\d)(?![^<]*>)/g, (_, num) =>
    `<a class="ref-link" data-doc="lpd" data-sec="${num}">${num}</a>`
  );
}

const GAME_NOTE_PATTERNS = [
  /\bVe\s+\d{4,5}\b/i, /\bve\s+\d{4,5}\b/i,
...
// ─── Section detection for numbered sections ──────────────────────────────────
function detectNumberedSection(para) {
  if (isHorizontalRule(para)) return null;
  ...
  // Inline: "XXXX text…" (number glued or with space)
  const inlineM = /^(\d{4,5})\s*([\s\S]+)$/.exec(plain);
  if (inlineM) {
...
function stripNumPrefix(runs, num) {
  ...
  return runs.map(run => {
    ...
    const newSegs = run.segments.map(seg => {
      if (!stripped && seg.type==='text' && seg.text.replace(/\s/,'').startsWith(num)) {
        stripped = true;
        return { ...seg, text: seg.text.replace(new RegExp('^\\s*' + num + '\\s*'), '') };
      }
      return seg;
    });
...
const order = [];
let current = null;
let inNumberedPhase = false;

// First 3 paragraphs are TOC headers — skip them.
// Paragraphs 3+ are either narrative (→ α section) or numbered sections (0001+).
for (let i = 0; i < paras.length; i++) {
  const para = paras[i];
  if (isHorizontalRule(para)) continue;

  if (i < 3) continue; // skip TOC

  if (!inNumberedPhase) {
    // Check if this is the start of the numbered sections
    const sec = detectNumberedSection(para);
    if (sec) {
      inNumberedPhase = true;
      // Start numbered section
      if (!sections[sec.num]) { sections[sec.num] = []; order.push(sec.num); }
      current = sec.num;
      if (sec.type === 'inline') {
        const rawText = sec.restRuns.map(r=>r.segments.map(s=>s.text||'').join('')).join('').trim();
        if (rawText) {
          let html = runsToHtml(sec.restRuns);
          html = linkifyRefs(html);
          if (html.trim()) {
    ...
    // Not a numbered section — add to α
    if (!sections['α']) { sections['α'] = []; order.unshift('α'); }
    current = 'α';
    const h = paraToHtml(para);
    if (h) sections['α'].push(h);
  } else {
    // In numbered phase
    const sec = detectNumberedSection(para);
    if (sec) {
...
// Sort order: α first, then numeric
order.sort((a, b) => {
...
console.log('Sections:', order.join(', ').substring(0, 120));
...
const output = [
  ...
  `// Auto-generated by parse_lpd.js on ${new Date().toISOString()}`,
  `// Sections: ${sectionCount}`,
  ``,
  `var LPD_DATA = ${JSON.stringify(DATA, null, 2)};`,
  ``,
].join('\n');
...
console.log('\n=== Spot-checks ===');
['α','0001','0010','0058'].forEach(k => {
  const p = sections[k];
  if (p) console.log(`[${k}] ${p.length} paras — "${(p[0]||'').replace(/<[^>]+>/g,'').substring(0,70)}"`);
  else   console.log(`[${k}] NOT FOUND`);
});
if (sections['α']) console.log(`α section has ${sections['α'].length} paragraphs`);
console.log('\nDone!');
