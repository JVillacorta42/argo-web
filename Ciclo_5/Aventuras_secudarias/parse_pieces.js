...
/**
 * parse_pieces.js
 * Parses Pieces.docx (pieces_doc.xml) → pieces_data.js
 * Run: node "Ciclo_5/Aventuras_secudarias/parse_pieces.js"
 *
 * Structure:
 *   α  — Intro section (Ramas de Olivo + all chapter intros, paras before 0001)
 *   0001–0057 — numbered game sections
 */

const fs   = require('fs');
const path = require('path');

const XML_PATH  = path.join(__dirname, 'pieces_doc.xml');
const OUT_PATH  = path.join(__dirname, 'pieces_data.js');
const IMGS_DIR  = 'Aventuras_secudarias/pieces_img';
...
try {
  ...
  const relsXml = execSync(`unzip -p "${path.join(__dirname, '..', 'Pieces.docx')}" word/_rels/document.xml.rels`).toString();
...
function linkifyRefs(html) {
  return html.replace(/(?<![&\w="])(\d{4,5})(?!\d)(?![^<]*>)/g, (_, num) =>
    `<a class="ref-link" data-doc="pieces" data-sec="${num}">${num}</a>`
  );
}
...
const GAME_NOTE_PATTERNS = [
  ...
  /^Independientemente/i, /^De lo contrario/i,
  /^Protectorado:/i, /^Seguidores?:/i, /^Vanguardia:/i,
  /^Obtén\b/i, /^Reduce\b/i, /^Resuelve\b/i,
  /Pista\s+[A-Z]/i, /^Marca\s+AA/i,
  /regresa a la Fase/i,
];
...
// Everything before 0001 goes into 'α' (the intro section)
let current = 'α';
sections['α'] = [];
order.push('α');
...
const output = [
  ...
  `// Auto-generated by parse_pieces.js on ${new Date().toISOString()}`,
  `// Sections: ${sectionCount}`,
  ``,
  `var PIECES_DATA = ${JSON.stringify(DATA, null, 2)};`,
  ``,
].join('\n');
...
// Spot-checks
console.log('\n=== Spot-checks ===');
['α','0001','0010','0030','0057'].forEach(k => {
  const p = sections[k];
