...
/**
 * parse_toyd.js
 * Parses The One You Don't.docx (toyd_doc.xml) → toyd_data.js
 * Run: node "Ciclo_5/Aventuras_secudarias/parse_toyd.js"
 *
 * Structure:
 *   α  — Intro section (everything before 0001, incl. all chapter intros)
 *   0001–0077 — numbered game sections
 */

const fs   = require('fs');
const path = require('path');

const XML_PATH  = path.join(__dirname, 'toyd_doc.xml');
const OUT_PATH  = path.join(__dirname, 'toyd_data.js');
const IMGS_DIR  = 'Aventuras_secudarias/toyd_img';
...
try {
  ...
  const docx = path.join(__dirname, "The One You Don't.docx");
...
function linkifyRefs(html) {
  return html.replace(/(?<![&\w="])(\d{4,5})(?!\d)(?![^<]*>)/g, (_, num) =>
    `<a class="ref-link" data-doc="toyd" data-sec="${num}">${num}</a>`
  );
}
...
const output = [
  ...
  `// Auto-generated by parse_toyd.js on ${new Date().toISOString()}`,
  `// Sections: ${sectionCount}`,
  ``,
  `var TOYD_DATA = ${JSON.stringify(DATA, null, 2)};`,
  ``,
].join('\n');
...
// Spot-checks
console.log('\n=== Spot-checks ===');
['α','0001','0010','0040','0077'].forEach(k => {
  const p = sections[k];
